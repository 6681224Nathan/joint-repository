...
SELECT * FROM employee
WHERE lastName > "Henderson"; #in alphebetical order, the last name that is above henderson will be shown

CREATE TABLE newemp (
SELECT * FROM employee
); #dupilcate data into other table

CREATE TABLE newemp2 (
SELECT * FROM employee where salary >30000
); #dupilcate data into other table with condition, filter only some parts out
#new view table can also be created as well

#name varchar(30) NOT NULL;
#name varchar(30) UNIQUE
#CONSTRAINT uc_PersonID UNIQUE (P_Id,LastName)) works with primary key as well, combining two attr
...
CREATE TABLE Persons #DONT EXECUTE conditional for attribute
( 
