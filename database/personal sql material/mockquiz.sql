...
ALTER TABLE Customers ADD CONSTRAINT CustomerID_FK FOREIGN KEY (AccountTypeID) REFERENCES accounttypes(AccountTypeID);


 Select c.CustomerID, c.CustomerName, d.Total_Deposit, w.Total_Withdraw, (Total_Deposit - Total_Withdraw) as Difference
 From Customers AS c LEFT JOIN 
 (Select CustomerID, SUM(WithdrawalAmount) as Total_Withdraw From withdrawals group by CustomerID) as w on c.CustomerID=w.CustomerID
 LEFT JOIN
 (Select CustomerID, SUM(DepositAmount) as Total_Deposit From deposits group by CustomerID) as d on c.CustomerID=d.CustomerID;
 
 
 Select e.EmployeeID, e.FirstName, d.No_Deposits, w.No_Withdrawals, (d.No_Deposits + w.No_Withdrawals) as Total_Trans
 From employees AS e LEFT JOIN 
 (Select EmployeeID, Count(*) as No_Withdrawals From withdrawals group by EmployeeID) as w on e.EmployeeID=w.EmployeeID
 LEFT JOIN
 (Select EmployeeID, Count(*) as No_Deposits From deposits group by EmployeeID) as d on e.EmployeeID=d.EmployeeID;
 
Select c.state, count(*) as Tol_Customer From Customers as c group by state;

Select CustomerName From customers where CustomerName like "% % H%";
